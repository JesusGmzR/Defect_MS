...
import { verificarToken, verificarInspectorQA } from '../middleware/auth';
import {
 ...
 */
router.get('/pendientes', verificarToken, verificarInspectorQA, async (_req: AuthenticatedRequest, res: Response) => {
  try {
 ...
 * Aprobar reparación
 */
router.post('/:repair_id/aprobar', verificarToken, verificarInspectorQA, async (req: AuthenticatedRequest, res: Response<SuccessResponse | ErrorResponse>) => {
  const connection = await pool.getConnection();
  
  try {
    const { repair_id } = req.params;
    const { observaciones_qa } = req.body as ValidacionQARequest;
    const inspector = req.user?.nombre_completo || req.user?.username || 'Unknown';
    
    // Iniciar transacción
    await connection.beginTransaction();
    
    // Verificar que la reparación existe y está en estado correcto
    const [repairRows] = await connection.execute<any>(
    ...
    if (repair.status !== 'Reparado') {
      ...
      res.status(400).json({ 
        ...
        details: `Estado actual: ${repair.status}`
      });
      return;
    }
    ...
    if (repair.inspeccionado_por_qa) {
      ...
      res.status(400).json({ 
        ...
        details: `Resultado: ${repair.resultado_inspeccion_qa}`
      });
      return;
    }
 ...
 * Rechazar reparación
 */
router.post('/:repair_id/rechazar', verificarToken, verificarInspectorQA, async (req: AuthenticatedRequest, res: Response<SuccessResponse | ErrorResponse>) => {
  const connection = await pool.getConnection();
 ...
 */
router.get('/historial', verificarToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { dias = '30', inspector } = req.query as { dias?: string; inspector?: string };
    ...
    const params: any[] = [parseInt(dias)];
 ...
 */
router.get('/estadisticas', verificarToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
